...
replicateA c fa
  | c <= 0 = pure Nil
  | otherwise =  pure (:.) <*> fa <*> replicateA (c-1) fa
...
filtering p (h:.t) = lift2 (\val flag -> if flag then (val:.) else id) (pure h) (p h) <*> filtering p t
