...
instance Functor ListZipper where
  fn <$> ListZipper l f r = ListZipper (fn <$> l) (fn f) (fn <$> r) 
...
toList (ListZipper (x:.t) f r) = toList $ ListZipper t x (f :. r)
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus fn (ListZipper l f r) = ListZipper l (fn f) r
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft (ListZipper (h:.t) p r) = IsZ $ ListZipper t h (p:.r)
moveLeft _ = IsNotZ
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight (ListZipper l p (h:.t)) = IsZ $ ListZipper (p:.l) h t
moveRight _ = IsNotZ
...
swapLeft ::
  ListZipper a
  -> MaybeListZipper a
swapLeft (ListZipper (h:.t) p r) = IsZ $ ListZipper (p:.t) h r
swapLeft _ = IsNotZ
...
swapRight ::
  ListZipper a
  -> MaybeListZipper a
swapRight (ListZipper l p (h:.t)) = IsZ $ ListZipper l h (p:.t)
swapRight _ = IsNotZ
...
dropLefts ::
  ListZipper a
  -> ListZipper a
dropLefts (ListZipper _ p r) = ListZipper Nil p r
...
dropRights ::
  ListZipper a
  -> ListZipper a
dropRights (ListZipper l p _) = ListZipper l p Nil
...
moveLeftN ::
  ...
  -> MaybeListZipper a
moveLeftN 0 l = IsZ l
moveLeftN n t@(ListZipper l f r) = if n <= 0 then moveRightN (-n) t else
    IsZ $ ListZipper (drop n l) (headOr undefined (drop (n-1) l) ) ((reverse $ take (n-1) l) ++ (f :. r))
  -- todo very ugly
...
moveRightN n t@(ListZipper l f r) = if n <= 0 then moveLeftN (-n) t else
    IsZ $ ListZipper ((reverse $ take (n-1) r) ++ (f :. l)) (headOr undefined (drop (n-1) r) ) (drop n r)  
  -- todo very ugly
...
moveLeftN' n t@(ListZipper l _ _) = case moveLeftN n t of
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n t@(ListZipper _ _ r)= case moveRightN n t of
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft v (ListZipper l f r) = ListZipper (f:.l) v r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight v (ListZipper l f r) = ListZipper l v (f:.r)
...
-- /Tip:/ Use @List#repeat@.
  pure a = ListZipper inf a inf where inf = repeat a
-- /Tip:/ Use `zipWith`
  (ListZipper l p r) <*> (ListZipper lv pv rv) = ListZipper (zipWith ($) l lv) (p pv) (zipWith ($) r rv)
...
instance Traversable ListZipper where
  ...
  traverse fn (ListZipper l f r) = pure ListZipper <*> traverse fn l <*> fn f <*> traverse fn r
...
instance Traversable MaybeListZipper where
  ...
  traverse afb (IsZ lz) = IsZ <$> traverse afb lz
