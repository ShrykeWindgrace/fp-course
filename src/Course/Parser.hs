...
valueParser ::
  a
  -> Parser a
valueParser a = P $ \inp -> Result inp a
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  klei =<< P cont = P $ \inp -> case cont inp of
        Result i a -> parse (klei a) i
...
list ::
  ...
  -> Parser (List a)
list p = list_ where
  list_ = list1_ ||| pure Nil
  list1_ = lift2 (:.) p list_
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser  = foldRight appl (pure Nil) where
    appl h t = do
      x <- h
      xs <- t
      pure (x:.xs)
