...
hex ::
  Parser Char
hex = do
  lst <- replicateA 4 $ satisfy isHexDigit
  case readHex lst of
    Empty -> error "tzy1"
    Full x -> pure $ chr x
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby val sep = sepby1 val sep ||| pure Nil
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma o c val = betweenCharTok o c $ sepby val $ charTok ','
